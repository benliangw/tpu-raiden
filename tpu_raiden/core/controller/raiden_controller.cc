...
absl::Status RaidenController::Init(
  ...
  for (size_t i = 0; i < worker_addresses.size(); ++i) {
    ...
    if (!status.ok()) {
      return absl::Status(
          status.code(),
          absl::StrCat("Failed to register static worker ",
                       worker_addresses[i], ": ", status.message()));
    }
  }
  ...
  if (!raiden_orchestrator_address.empty()) {
    ...
    if (!status.ok()) {
      return absl::Status(status.code(),
                          absl::StrCat("Failed to register with orchestrator: ",
                                       status.message()));
    }
  }
...
absl::StatusOr<std::unique_ptr<RaidenController>> RaidenController::Create(
  ...
  return Create(unit, /*worker_addresses=*/absl::Span<const std::string>(),
...
absl::StatusOr<std::unique_ptr<RaidenController>> RaidenController::Create(
  ...
  RETURN_IF_ERROR(controller->Init(worker_addresses,
