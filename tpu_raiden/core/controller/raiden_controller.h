...
class RaidenController {
 public:
  ...
  // As above, with statically supplied worker addresses that register during
  // initialization (they count toward `expected_worker_count`).
  static absl::StatusOr<std::unique_ptr<RaidenController>> Create(
 ...
 private:
  // Performs only the never-failing member initialization; Create() runs
  // Init() afterwards, which does all the fallible work.
