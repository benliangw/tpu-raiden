...
def _lookup_ids(controller, req_id, uuid, units, claim_owner=None):
  ...
  return {
      unit: list(registration.block_ids)
      for unit, registration in controller._lookup_request_blocks(
          req_id, uuid, units, claim_owner=claim_owner
      ).items()
  }


def _dense_byte_registration(
    num_blocks, *, page_tokens=4096, tail_tokens=None, token_bytes=1024
):
...
class RaidenControllerTest(absltest.TestCase):
  ...
  def test_stage3_rejects_invalid_registration_geometry(self):
    ...
    with self.assertRaisesRegex(ValueError, "page_tokens must be positive"):
      controller.register_work_unit(
          unit,
          ["127.0.0.1:8000"],
          control_plane_rpc_address="127.0.0.1:9000",
  ...
  def test_stage3_block_argument_validation_is_atomic(self):
    ...
    controller.register_request_blocks(
        "bad", 123, src_units[0], [1], pool_spans=[_dense_byte_registration(1)]
    )
  ...
  def test_pool_tag_selection_is_request_data(self):
    ...
    with self.assertRaisesRegex(ValueError, "do not match any registered pool"):
      controller._build_pool_reshard_plan(
          ...
          dst_device_block_ids=dst_ids,
          num_tokens=65536,
          transfer_pool_tags=["not-a-pool"],
          parallelism=8,
      )
    with self.assertRaisesRegex(ValueError, "at least one pool tag"):
      controller._build_pool_reshard_plan(
          src_units=src_units,
          dst_units=[dst_unit],
          src_metadata=controller._get_local_metadata(src_units),
    ...
    # The claim was rolled back both times, so a fresh plan still succeeds
    # and the selection drives both the plan and the skip summary.
    plan = controller._build_pool_reshard_plan(
        src_units=src_units,
        dst_units=[dst_unit],
        src_metadata=controller._get_local_metadata(src_units),
  ...
  def test_num_tokens_beyond_declared_prompt_is_refused(self):
    controller, _, src_units, dst_unit, _ = self._stage3_fixture(
        num_tokens=1024
    )
    ...
    with self.assertRaisesRegex(ValueError, "has no declared coverage"):
      controller._build_pool_reshard_plan(
          ...
          req_id="request",
          uuid=123,
          dst_device_block_ids=over_length_pages,
          num_tokens=2048,
          transfer_pool_tags=["fa"],
          parallelism=8,
      )
  ...
  def test_stage3_planner_failure_abandons_its_request_block_claim(self):
    ...
    # Rank 0 declares only its first block's spans, leaving a hole in the
  ...
  def test_raw_legacy_release_without_force_is_rejected(self):
    ...
    request = raiden_controller.controller_service_pb2.ControllerRequest(
        command=(
            raiden_controller.controller_service_pb2.ControllerRequest.COMMAND_RELEASE_REQUEST_BLOCKS
        ),
        release_request_blocks_request=(
            raiden_controller.controller_service_pb2.ReleaseRequestBlocksRequest(
    ...
    try:
      with self.assertRaisesRegex(
        ...
        facade._send_protobuf_rpc(request)
    finally:
      server.stop()
  ...
  def test_request_block_registry_ttl_and_worker_restart_cleanup(self):
    ...
    with mock.patch.object(
      ...
      with self.assertRaisesRegex(ValueError, "Missing producer block"):
        _lookup_ids(controller, "expired", 77, [unit])
    ...
    with mock.patch.object(
      ...
      with self.assertRaisesRegex(ValueError, "Missing producer block"):
        _lookup_ids(controller, "restart", 78, [unit])
  ...
  def test_worker_restart_cannot_replace_a_claimed_snapshot(self):
    ...
    self.assertEqual(
        _lookup_ids(controller, "claimed-restart", 79, [unit]),
        {unit: [10]},
    )
  ...
  def test_request_block_cancellation_wins_and_is_idempotent(self):
    ...
    with self.assertRaisesRegex(ValueError, "cancelled"):
      _lookup_ids(controller, "cancelled", 81, [unit])
  ...
  def test_request_block_cancellation_seals_late_registration(self):
    ...
    controller.register_request_blocks("late", 82, unit, [3])
    self.assertEqual(
        _lookup_ids(controller, "late", 82, [unit]),
        {unit: [3]},
    )
  ...
  def test_request_block_lookup_claim_wins_over_cancellation(self):
    ...
    self.assertEqual(
        _lookup_ids(controller, "claimed", 83, [unit]),
        {unit: [4]},
    )
    self.assertFalse(
  ...
  def test_request_block_release_waits_for_all_claimed_units(self):
    ...
    self.assertEqual(
        _lookup_ids(controller, "aggregate", 86, units),
        {units[0]: [0], units[1]: [1]},
    )
    self.assertEqual(
  ...
  def test_request_block_terminal_vote_before_claim_keeps_d5_snapshot(self):
    ...
    self.assertEqual(
        _lookup_ids(controller, "preclaim-terminal", 87, units),
        {units[0]: [], units[1]: [4]},
    )
    self.assertEqual(
        controller.complete_request_blocks(
  ...
  def test_nonempty_request_block_cannot_complete_before_claim(self):
    ...
    self.assertEqual(
        _lookup_ids(controller, "premature-terminal", 89, [unit]),
        {unit: [3]},
    )
  ...
  def test_claim_refreshes_preclaim_terminal_vote_ttl(self):
    ...
    with mock.patch.object(
      ...
      self.assertEqual(
          _lookup_ids(controller, "late-claim", 88, units),
          {
              units[0]: [],
              units[1]: [1],
          },
  ...
  def test_request_block_lifecycle_markers_expire(self):
    ...
    with mock.patch.object(
      ...
      controller.register_request_blocks("cancel-expiry", 84, unit, [6])
      self.assertEqual(
          _lookup_ids(controller, "cancel-expiry", 84, [unit]),
          {unit: [6]},
      )
    ...
    with mock.patch.object(
      ...
      _lookup_ids(controller, "claim-expiry", 85, [unit])
      self.assertTrue(
  ...
  def test_request_block_lookup_cancel_race_has_single_winner(self):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
      for generation in range(25):
        ...
        def lookup(b=barrier, r=req_id, u=uuid):
          b.wait(timeout=5)
          try:
            return _lookup_ids(controller, r, u, [unit])
          except ValueError as error:
            if "cancelled" not in str(error):
              raise
            return None

        def cancel(b=barrier, r=req_id, u=uuid):
          b.wait(timeout=5)
          return controller.cancel_request_blocks_if_unclaimed(r, u)
