...
class LogicalBlockManager {
 public:
  ...
  // Allocates exactly the given block IDs, locked (the same state
  // Allocate(n, /*lock=*/true) would have produced for them). All IDs must
  // be in range, currently unallocated, and unique; on any violation an
