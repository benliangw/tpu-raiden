...
class KVCacheStore:
  ...
  def __init__(
    ...
    Args:
      ...
      l3_spill_dst_raiden_id: When set, this store autonomously hands its
        coldest host-resident blocks to that peer via a remote save once
      ...
      l3_spill_max_blocks_per_batch: Kernel blocks per spill batch.
  ...
  def lookup(
    ...
    By default PINS every hash it returns, one pin each, so the answer cannot
    be evicted between being given and being used. The operation the caller
    goes on to perform consumes that pin: load() drops it on a successful
    local load, and save() drops it on success. A caller that does neither --
    one that only wanted to know what is resident -- must release() what it
    was given, or pass pin_found=False to observe without acquiring (a pure
    peek: no pin, no recency perturbation).
    ...
    Args:
      ...
      pin_found: Whether to pin local hits. Defaults to True.
  ...
  def evict(self, block_hashes: list[bytes]) -> int:
    ...
    under admission pressure: a node that has handed blocks to a peer via a
    remote save() (proactive eviction) uses this to free its local copies
    once poll_save_status() reports them done.
  ...
  def get_evictable_keys(self, count: int) -> list[bytes]:
    ...
    not perturbed. Intended for planning proactive eviction (a remote save
    to a peer). Candidates are lookup-invisible and unreadable to peers, so
  ...
  def poll_save_status(
    ...
    batch -- it re-published those to the global registry before answering
    (an offer it cannot vouch for fails instead), so like `done` they are
    findable on the peer and the caller may drop its own copy of exactly
    those hashes. `unregistered` is a destination that HAS the bytes but
    could not publish them, so no lookup will find them there; the safe
    default is to keep your own copy.
    """
