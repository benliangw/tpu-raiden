...
absl::Status KVCacheStore::SaveRemote(
  ...
  return SaveRemoteInternal(block_hashes, dst_raiden_id, /*l3_spill=*/false);
}

absl::Status KVCacheStore::SaveRemoteInternal(
  ...
  if (l3_spill) {
  ...
  } else {
    {
      ...
      for (size_t i = 0; i < slices.size(); ++i) {
        ...
        // the caller's pin was separate and might not exist at all -- which
        // made "a save consumes one pin" unstatable for half the API.
        if (backend->GetPinCount(hash) <= 0) {
          return absl::FailedPreconditionError(absl::StrCat(
...
void KVCacheStore::FinishRemoteWrite(const RemoteWriteState& state,
  ...
  if (auto* backend = this->backend().get(); backend != nullptr) {
    ...
    if (succeeded && !state.l3_spill) {
      ...
      // An L3 spill batch has no caller: its per-hash resolve pin is the one
      // released above, so a second release would drop a pin someone else
      // holds.
      //
...
void KVCacheStore::MaybeSpillL3() {
  ...
  RaidenId dst;
  {
    ...
    // A hash some other remote save is already shipping would fail the
  ...
  absl::Status status = SaveRemoteInternal(batch, dst, /*l3_spill=*/true);
  if (!status.ok()) {
    absl::MutexLock lock(mutex_);
    l3_spill_in_flight_ = false;
    l3_next_spill_ = absl::Now() + l3_spill_options_->cooldown;
...
void KVCacheStore::PollRemoteWritesInternal(bool l3_spill) {
  ...
  // operation now owns) and reporting every hash twice. The l3_spill filter
  // routes each operation to exactly one consumer: the completion poller
  // takes the public saves, the L3 spill thread takes its own, and neither
  // can stall the other on a slow peer.
  std::vector<RemoteWriteState> to_poll;
  {
    absl::MutexLock lock(mutex_);
    for (const auto& state : active_remote_writes_) {
      if (state.l3_spill != l3_spill) {
        continue;
      }
